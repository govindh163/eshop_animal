...
function fetch_product($user_id = NULL, $filter = NULL, $id = NULL, $category_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $return_count = NULL, $is_deliverable = NULL)
{
    ...
    $t->db->select($discount_filter_data . '(select count(id)  from products where products.category_id=c.id ) as total,count(p.id) as sales, p.stock_type , p.is_prices_inclusive_tax, p.type ,GROUP_CONCAT(DISTINCT(pa.attribute_value_ids)) as attr_value_ids,p.id,p.stock,p.name,p.category_id,p.short_description,p.slug,p.description,p.total_allowed_quantity,p.deliverable_type,p.deliverable_zipcodes,p.minimum_order_quantity,p.quantity_step_size,p.cod_allowed,p.row_order,p.rating,p.no_of_ratings,p.image,p.is_returnable,p.is_cancelable,p.cancelable_till,p.indicator,p.other_images, p.video_type, p.video, p.tags, p.warranty_period, p.guarantee_period, p.made_in,p.sold_by,p.pur_price,p.availability,c.name as category_name,tax.percentage as tax_percentage,tax.id as tax_id ')->join(" categories c", "p.category_id=c.id ", 'LEFT')
    ...
    if (!empty($product)) {
        ...
        for ($i = 0; $i < count($product); $i++) {
            ...
            $product[$i]['sold_by'] = isset($product[$i]['sold_by']) && !empty($product[$i]['sold_by']) ? $product[$i]['sold_by'] : '';
            $product[$i]['pur_price'] = isset($product[$i]['pur_price']) && !empty($product[$i]['pur_price']) ? $product[$i]['pur_price'] : '';
